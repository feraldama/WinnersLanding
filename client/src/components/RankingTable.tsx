  ...
  useEffect(() => {
    const loadCompetencias = async () => {
      try {
        ...
        // Mapear Competencia[] a { CompetenciaId, CompetenciaNombre }[]
        const competenciasMapeadas = (response.data || []).map((comp) => ({
          CompetenciaId: comp.id,
          CompetenciaNombre: comp.nombre,
        }));
        setCompetencias(competenciasMapeadas);
        if (competenciasMapeadas.length > 0) {
          setCompetenciaSeleccionada(
            competenciasMapeadas[0].CompetenciaId.toString()
          );
        }
      } catch (error) {
            ...
            >
              {competencias.map((competencia) => (
                <option
                  ...
                  value={competencia.CompetenciaId.toString()}
                >
